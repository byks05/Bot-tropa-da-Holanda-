...
if (sub === "entrar") {
  ...
  let res = await pool.query(
    "SELECT ativo, entrada, canal FROM pontos WHERE user_id = $1",
    [userId]
  );
  ...
  if (userData.ativo) return message.reply("❌ Você já iniciou seu ponto.");

  // Atualiza ativo e entrada no banco
  await pool.query(
